<?php echo form_open('clerk/advanced_add_child_controller/search'); ?>
<div class="search1"><h3>Validate by mother's ID Number</h3>
	<?php
...
<input type="text" name="idno" placeholder="Enter Mothers ID Number" required><br>
			<input type="submit" name="btnSave" class="btn btn-primary" value="Validate"><br></div>
		...
		if(isset($result)){
			foreach($result as $searchUsers){
				...
				<tr>
					...
					<td>
			<a href="<?php echo base_url('clerk/advanced_add_child_controller/advancedAdd/'.$searchUsers->ID); ?>" class="btn btn-info">Add a child</a>
