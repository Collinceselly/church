...
class IndividualTithesReportModel extends CI_Model
{
    ...
    public function churchTithes($date = '', $date2 = '')
    {
        $this->db->select('*');
        $this->db->where("SABBATH_DATE BETWEEN '" .$date. "' AND '". $date2."'", "", false);
        $query = $this->db->get("adults_givings");
    ...
    public function churchContributionsMonthly($date, $date2)
    {
        $q = "SELECT SUM(TITHES) AS TITHES,
                SUM(COMBINED_OFFERING) AS COMBINED_OFFERING,
                SUM(CAMP_OFFERING) AS CAMP_OFFERING,
                ...
                SUM(LOCAL_CHURCH) AS LOCAL_CHURCH,
                SUM(STATION_DEVELOPMENT) AS STATION_DEVELOPMENT,
                DATE_FORMAT(SABBATH_DATE, '%m-%Y') AS SABBATH_DATE, NAME FROM adults_givings WHERE DATE_FORMAT(SABBATH_DATE, '%m-%Y') BETWEEN '$date' and '$date2' GROUP BY DATE_FORMAT(SABBATH_DATE, '%m-%Y')";
    ...
    public function churchContributionsYearly($date, $date2)
    {
        $q = "SELECT SUM(TITHES) AS TITHES,
                SUM(COMBINED_OFFERING) AS COMBINED_OFFERING,
                SUM(CAMP_OFFERING) AS CAMP_OFFERING,
                ...
                SUM(LOCAL_CHURCH) AS LOCAL_CHURCH,
                SUM(STATION_DEVELOPMENT) AS STATION_DEVELOPMENT,
                DATE_FORMAT(SABBATH_DATE, '%Y') AS SABBATH_DATE, NAME FROM adults_givings WHERE DATE_FORMAT(SABBATH_DATE, '%Y') BETWEEN '$date' and '$date2' GROUP BY DATE_FORMAT(SABBATH_DATE, '%Y')";
